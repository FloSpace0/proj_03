...
import heapq #libreria per poter personalizzare la PriorityQueue
from geopy.distance import geodesic #libreria che ci aiuta per calcolare la distanza. Visto che la terra non è piatta 
import pandas as pd #libreria per leggere i files cvc
import os
...
class PathFinding(SearchProblem):
    ...
    def get_country_from_coordinates(self, lat, lon):
        # In un'aplicazione  reale questa funzione dovrebbe determinare il paese 
        # in base alle coordinate usando un servizio di geocoding
        # Per semplicità, usiamo un valore predefinito

        #Ad ogni modo è solo per esempio, nel progetto non farà differenza avercelo o meno
        # ma sarebbe più user friendly
        return "Italy"
    ...
    def getSuccessors(self, state) -> list:
        ...
        Ogni successore rappresenta un'offerta di lavoro.
        """
        successori = []
    ...
    def calculate_cost(self, current_state, new_state, w):
        """
        Calcola il costo di passare dallo stato corrente al nuovo stato.
        Il costo sarebbe la funzione che determina la PriorityQueue.
        Più basso sarà il costo e prima dovrebbe uscire il lavoro una volta che tireremo i migliori fuori.
        """
        job = new_state['job']
        ...
        #Questo è il guadagno, va in base alla distanza dal punto iniziale al punto prossimo e sarà 0 se la persona
        #non va a cercare un lavoro in base alla distanza, ma priorio ha intenzione di trasferirsi
        g = distanza * 1000 * w

        #Questa è l'heuristica, va in base al guadagno stimato futuro in base alle condizioni di vita e al salario
    ...
    def isGoal(self, state) -> bool:
        """
        In questo problema non abbiamo un vero goal state, vogliamo trovare i migliori lavori.
        Per A* dovremo usare una versione modificata dell'algoritmo, in quanto tipicamente un A* ha un goal finale.
        ma nel nostro caso non sappiamo quale possa essere, e dobbiamo cercarla ma utilizzando le tecniche come l'heuristica
        """
    ...
    def find_best_jobs(self, w, top_n=5):
        """
        ...
        for _, job in self.available_jobs.iterrows():
            ...
            costo = self.calculate_cost(self.init, job_state, w)
